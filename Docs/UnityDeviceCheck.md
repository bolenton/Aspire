...
- Final build **12 passed** the real-microphone mode check once the phone stayed active. Capture began in **1.92 s**, listening resumed after the authored reply in **0.85 s**, and the corrected 0.7-second drag moved **1.43 m** while mode stayed on. Explicit off stopped additional PCM frames. Activation p95 was **17.14 ms**, movement p95 **16.72 ms**, and reply/resume p95 **16.73 ms**; no sampled voice-phase frame exceeded 50 ms. Two earlier attempts were interrupted when the app entered the background before capture.
- Final build **12 also passed all 21 activities on Orange**, using actual navigation, world taps, dialogue choices and chime buttons, including both bridge crossings, Luma, the moon harp, moon seed, nest restoration and free exploration. A short 20-second playthrough frame sample averaged 59.4 fps with 17.04 ms p95. Evidence: `iphone-playthrough-build12.log` and `iphone-orchard-build12.png`.
- All four GitHub CI jobs passed for `78afabf`; the local final core run passed all 20 checks. Normal-save readback preserved quest progress, inventory, journals and avatar; only the last-played timestamp changed after normal launch. The iPad remains unavailable. Development probes use a separate verification vault, and normal launch must be restored afterward.

Evidence: ignored `.artifacts/unity/iphone-voice-mode-build11.log`, `voice-listening-build11.png`, `xcodebuild-build12.log`, `iphone-install-build12.json`, `iphone-apps-build12.json` and `iphone-voice-mode-build12-final.log`, `voice-listening-build12.png` and before/after save copies.
