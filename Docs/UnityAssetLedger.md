...
## Lantern Orchard expansion

Six additional original Blender exports are generated by `scripts/art/orchard.py`: `Orchard`, `OrchardGate`, `Luma`, `MoonHarp`, `MoonSeed`, and `LanternNest`. The Orchard uses silver-blue leaf canopies, a pale outlined path, lanterns and broad activity clearings. Luma has a modeled face, talons, scarf, wings and lantern, and calmly turns toward a nearby explorer. Ember adds named ear and jaw pivots for listening and speech amplitude animation. All are original procedural artwork, with source included; no marketplace model license is required.

The expansion retains the newly sourced meadow audio from the Unity port. Live dialogue and game narration use the family's Piper voice when connected. The Moon Harp uses the existing untimed chime instrument with a new five-note song. Firefly motion is slow and non-flashing. These are authored game assets, not a claim of completed AAA production quality.
